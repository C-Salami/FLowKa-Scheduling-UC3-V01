...
# ============================ CSS / LAYOUT ============================
# Single page: 85% Gantt, 15% control bar. No scrolling.
css = """
<style>
/* Remove default paddings and scrollbars */
html, body, [data-testid="stAppViewContainer"], .block-container {
  height: 100vh; overflow: hidden; padding: 0 !important; margin: 0 !important;
}
...
.bottom-wrap {
  height: 15vh;
  min-height: 110px; /* keeps it usable on short screens */
  border-top: 1px solid #eee;
  background: linear-gradient(to top, rgba(255,255,255,0.98), rgba(255,255,255,0.96));
  display: grid;
  grid-template-columns: 3fr 1fr; /* 75% transcript / 25% mic */
  gap: 16px;
...
/* Style the recorder widget button itself (single control) */
.mic-button, .mic-button * { box-sizing: border-box; }
...
.rec-wrap button:active, .rec-wrap [role="button"]:active {
  transform: scale(0.98);
}

/* Replace inner with visible mic glyph (inline SVG, guaranteed to render) */
...
/* Tiny debug link Streamlit shows sometimes (remove gaps) */
.block-container > :is(div, section):empty { display: none; }
</style>
"""
st.markdown(css, unsafe_allow_html=True)


# ============================ FILTERS (light, optional) =========================
# Keep filters minimal to save vertical space (expose only when needed).
...
with st.container():
    ...
    if sched.empty:
        ...
        gantt = (
            ...
            .properties(width="container", height= int(0.85 * 1000))  # visual height; container still drives 85vh
...
def _parse_duration_chunks(text: str):
    ...
    def numtok(tok: str):
        ...
        val = words_to_int(tok); return float(val) if val is not None else None
    for num, unit in re.findall(r"([\d\.,]+|\b[\w\-]+\b)\s*(days?|d|hours?|h|minutes?|mins?|m)\b", text, flags=re.I):
        n = numtok(num);  u = unit.lower()
...
def _extract_with_openai(user_text: str):
    ...
    SYSTEM = (
        ...
        "Return ONLY JSON matching the given schema. Supported intents: delay_order, move_order, swap_orders. "
        "Order IDs are 'Order N'. Convert relative dates to Asia/Makassar; default move time 08:00. "
        "If delay units missing, assume days; minutes allowed. Accept 'order five' and legacy 'O071'."
    )
    GUIDE = (
...
with st.container():
    ...
    if st.session_state.last_transcript:
        ...
        st.markdown(
            '<div class="transcript-box placeholder">(your transcript will appear here)</div>',
            unsafe_allow_html=True
        )

    # Mic button (25% width) – single control, restyled via CSS above
    st.markdown('<div class="mic-area"><div class="rec-wrap">', unsafe_allow_html=True)
    rec = mic_recorder(
        start_prompt="", stop_prompt="",
        key="press_mic", just_once=False, format="wav",
